...
- `Naver Trend Maker 10.exe`와 `desktop-artifacts/Naver Trend Maker 10 로컬버전.exe`를 바로 실행 가능한 최신 로컬 실행 파일로 맞췄습니다.
- 자동 순회를 중간부터 다시 돌릴 수 있도록 `여기부터 자동 시작` 버튼을 추가했습니다.
- 자동 순회 중 각 카테고리의 트렌드 분석 후보가 `Naver Trend Maker 10 베스트상품.xlsx`에 계속 누적되도록 정리했습니다.
- 로컬 API가 잠깐 재시작되어도 현재 카테고리와 진행 중인 run에서 기다렸다가 이어가도록 보강했습니다.
- 브랜드 제품 제외를 끄면 사용자 제외어도 같이 비워지도록 설정 적용 흐름을 정리했습니다.
